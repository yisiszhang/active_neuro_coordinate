...
% particular routine deals with the case of a = -1. Note that Canonical PDC
% for both values of a is identical.
%
...
A(6,7,1) =  .99;
A(9,8,1) =  .5;
A(4,10,1) = -1; % a = -1. Either -1 (Fig. 4,5) or +1 (Fig. 6) 
A(7,10,1) =  1;
...
set(h1supx, 'FontSize',[24],'FontWeight','bold','Color',[.4 .4 .4])
[ax,h1supy]=suplabel('Target','y',supAxes);
set(h1supy, 'FontSize',[24],'FontWeight','bold','Color',[.4 .4 .4])


eval(['print -depsc Figure_4.eps -painters'])
eval(['print -dpdf Figure_4.pdf -painters'])
...
%plot_canon(dc_c,'DC')%,max(max(max(len_c))))
h2 = plot_canon2(dc_c,'DC',T);
set(h2,'NumberTitle','off','MenuBar','none', ...
    'Name', ['Figure 5. cDC for Example 2 with a = -1'])
supAxes=[.08 .08 .84 .84];
[ax,h2supx]=suplabel('Source','x',supAxes);
set(h2supx, 'FontSize',[24],'FontWeight','bold','Color',[.4 .4 .4])
[ax,h2supy]=suplabel('Target','y',supAxes);
set(h2supy, 'FontSize',[24],'FontWeight','bold','Color',[.4 .4 .4])


eval(['print -depsc Figure_5.eps -painters'])
eval(['print -dpdf Figure_5.pdf -painters'])
